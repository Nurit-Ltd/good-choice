...
export function Collections({
  ...
  // Set default initial top scroll position on desktop mount
  useEffect(() => {
    ...
    <section className={`relative w-full overflow-hidden bg-secondary-800 ${className}`}>
      ...
      <div className="relative z-10 w-full min-h-160 lg:h-225 flex items-center justify-between py-16 lg:py-0 px-6 sm:px-12 lg:px-16 xl:pl-36 xl:pr-36.25">
        <div className="w-full flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 lg:gap-16 xl:gap-24">
          ...
          {/* Right Side Carousel Stream (Horizontal on Mobile 1.3 slides/view, Vertical on Desktop) */}
          <div className="relative w-full max-w-full lg:max-w-90 sm:lg:max-w-100 xl:max-w-105 flex flex-col items-start lg:items-center group/carousel lg:h-225 self-stretch">
            {/* Scrollable Container */}
            <div
              ref={scrollRef}
              className="w-full h-auto lg:h-225 overflow-x-auto lg:overflow-y-auto scrollbar-none flex flex-row lg:flex-col gap-6 py-2 lg:py-0 scroll-smooth snap-x lg:snap-none snap-mandatory"
              style={{
                scrollbarWidth: "none",
